...
function ResearchTab() {
  ...
  return (
    <div className="space-y-4">
      {(entries || []).length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No research data yet. Rhianna's findings will appear here.</p>}
      {(entries || []).map((e) => (
        <Card key={e.id}>
          <CardContent className="p-4">
            ...
            <pre className="text-sm whitespace-pre-wrap">{JSON.stringify(e.data, null, 2)}</pre>
          </CardContent>
        </Card>
      ))}
    </div>
