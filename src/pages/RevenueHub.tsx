  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Revenue', value: fmt.money(rev.total) },
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {(entries || []).length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No revenue data yet. Bot messages with financial data will appear here automatically.</p>}
          {(entries || []).map((e) => (
